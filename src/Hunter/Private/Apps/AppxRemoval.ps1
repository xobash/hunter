function Invoke-WithHunterAppxMutationLock {
    param([Parameter(Mandatory)][scriptblock]$Action)

    if ($null -eq (Get-Command -Name 'Invoke-WithNamedSemaphore' -ErrorAction SilentlyContinue)) {
        return (& $Action)
    }

    return (Invoke-WithNamedSemaphore -Name 'Global\HunterAppxMutation' -MaxConcurrency 1 -WaitTimeoutSeconds 1800 -Action $Action)
}
...
function Remove-AppxPatterns {
    ...
    return (Invoke-WithHunterAppxMutationLock -Action {
        $summary = [ordered]@{
            Success             = $true
            RemovedInstalled    = @()
            RemovedProvisioned  = @()
